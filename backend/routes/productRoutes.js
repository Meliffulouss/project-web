        ...
        if (sortBy) {
            switch (sortBy) {
                ...
                case "popularity":
                    sort = { rating: -1 };
                    break;
                default: // Add a default case to handle invalid sortBy values
                    sort = {}; // Or set a default sort order
                    break;
...
router.get("/best-seller", async (req, res) => {
    try {
        // Logic to fetch the product with the highest rating
        ...
        console.error("Error fetching best seller:", error); // Thêm log chi tiết
        res.status(500).json({ message: "Server Error", error: error.message }); // Trả về lỗi chi tiết
    }
});
...
router.get("/similar/:id", async (req, res) => {
    ...
    // Add logic to fetch similar products based on gender and category
    try {
